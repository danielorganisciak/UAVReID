...
class BigtoSmall(ImageDataset):

    dataset_dir = 'drone-reid/big-to-small'

    def __init__(self, root='', split_id=0, **kwargs):
        self.root = osp.abspath(osp.expanduser(root))
        self.dataset_dir = osp.join(self.root, self.dataset_dir)

        self.cam_a_dir = osp.join(self.dataset_dir, 'size_biggest')
        self.cam_b_dir = osp.join(self.dataset_dir, 'size_smallest')
        ...
        super(BigtoSmall, self).__init__(train, query, gallery, **kwargs)

    def prepare_split(self):
        if not osp.exists(self.split_path):
            print('Creating 10 random splits of train ids and test ids')

            cam_a_imgs = sorted(glob.glob(osp.join(self.cam_a_dir, '*.jpg')))
            cam_b_imgs = sorted(glob.glob(osp.join(self.cam_b_dir, '*.jpg')))
