...
__all__ = ['vit_base_patch16_224', 'vit_small_patch16_224', 'vit_base_resnet50d_224', 'vit_small_resnet50d_s3_224',
...
class VisionTransformer(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, loss='softmax', embed_dim=768, depth=12,
...
@register_model
def vit_small_patch16_224(num_classes, loss='softmax', pretrained=False, **kwargs):
    if pretrained:
        # NOTE my scale was wrong for original weights, leaving this here until I have better ones for this model
        kwargs.setdefault('qk_scale', 768 ** -0.5)
    model = VisionTransformer(num_classes=num_classes, loss=loss, patch_size=16, embed_dim=768, depth=8, num_heads=8, mlp_ratio=3., drop_rate=0.3, **kwargs)
...
@register_model
def vit_base_patch16_224(num_classes, loss='softmax', pretrained=False, **kwargs):
    model = VisionTransformer(
        num_classes=num_classes, loss=loss, patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,  qkv_bias=True,
...
@register_model
def vit_small_resnet50d_s3_224(num_classes, loss='softmax', pretrained=False, **kwargs):
    pretrained_backbone = kwargs.get('pretrained_backbone', True)  # default to True for now, for testing
    backbone = resnet50d(pretrained=pretrained_backbone, features_only=True, out_indices=[3])
    model = VisionTransformer(
        img_size=224, loss=loss, embed_dim=768, depth=8, num_heads=8, mlp_ratio=3, hybrid_backbone=backbone, **kwargs)
...
@register_model
def vit_base_resnet50d_224(num_classes, loss='softmax', pretrained=False, **kwargs):
    pretrained_backbone = kwargs.get('pretrained_backbone', True)  # default to True for now, for testing
    backbone = resnet50d(pretrained=pretrained_backbone, features_only=True, out_indices=[4])
    model = VisionTransformer(
        num_classes, loss=loss, img_size=224, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, hybrid_backbone=backbone, **kwargs)
